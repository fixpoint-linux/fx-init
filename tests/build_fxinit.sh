#!/bin/sh
# Build fx-init (U-C1 core). NO dhall-c, NO config.c.
cd "$(dirname "$0")/.."
mkdir -p build-tmp
...
cosmocc -std=c11 -O2 -g -Wall -Wextra -ffunction-sections -fdata-sections -Wl,--gc-sections $DEF $INC -o build-tmp/fx-init src/fx-init.c src/fx_supervise.c src/fx_reloc.c src/fx_probe.c src/fx_log.c $FXSTORE $ENGINE $DAFSA 2>build-tmp/fx-init.err
