// shell/mfe/fx-init-page.js — the fx-init docs pages (all 6 content pages)
// as ONE @mfe MFE module backed by a single Elm bundle.
//
// Six data-mfe slots (fx-init-landing, fx-init-boot, fx-init-supervise,
// fx-init-activate, fx-init-fxctl, fx-init-logs) all resolve to THIS module via
// the import map. The site is content-only (no browser playground), so unlike
// the shen/visage page modules there is no custom-element companion to import.
//
// The browser dedupes the module by its resolved URL, so there is ONE shared
// module instance; each slot mount calls mount() with a different element.
// The Elm app selects WHICH page to render from the `pathname` flag (the
// shell router pushes the path via history.pushState, so
// window.location.pathname always reflects the active sub-page).
//
// This mirrors visage-page.js's seam (fetch + scoped-eval of dist/elm.js),
// the only difference being the flags payload { pathname }.
//
...
function loadElm() {
  if (!elmPromise) {
    elmPromise = (async () => {
      ...
      if (!res.ok) {
        throw new Error(`fx-init-page: failed to fetch ${ELM_URL} (HTTP ${res.status})`);
      }
      const code = await res.text();
      const Elm = evalBundle(code);
      if (!Elm || !Elm.Main || typeof Elm.Main.init !== 'function') {
        throw new Error('fx-init-page: dist/elm.js did not expose Elm.Main.init');
      }
      return Elm;
    })().catch((err) => {
